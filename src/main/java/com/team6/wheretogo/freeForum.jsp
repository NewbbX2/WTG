...
//index No : 화면상에서 표기될 글번호.(실제 검색에서는 0부터라서-1된것을 찾음)
							...
							for(int indexNo=NoStart ;(indexNo>=1)&&(indexNo>NoNextStart);indexNo--){
								...
								<tr>
									<td><a href = "freeForum2.jsp?boardNo=<%=board.getBoardNo()%>" style="text-decoration:none;color:black"><%=indexNo%></a></td>	
									<td><a href = "freeForum2.jsp?boardNo=<%=board.getBoardNo()%>" style="text-decoration:none;color:black"><%=board.getBoardTitle()%></a></td>	
									<td><a href = "freeForum2.jsp?boardNo=<%=board.getBoardNo()%>" style="text-decoration:none;color:black"><%=board.getBoardEnrollDate()%></a></td>	
									<td><a href = "freeForum2.jsp?boardNo=<%=board.getBoardNo()%>" style="text-decoration:none;color:black"><%=board.getBoardWriter()%></a></td>
									<td><a href = "freeForum2.jsp?boardNo=<%=board.getBoardNo()%>" style="text-decoration:none;color:black"><%=board.getBoardViews()%></a></td>	
								</tr>
			...
			<div id = "BoardSearch">
				...
				if(boardList!=null&&boardList.size()!=0){
							...
							<div id="BoardPageNo">
								<a href="freeForum.jsp?pageNo=<%=1%>"><<</a>
									<%
									//시작번호 10넘어가면 현재번호부터 10개
									if(pageNo>10)
									{
										//-10돌아가는 < 추가.%>
										<a href="freeForum.jsp?pageNo=<%=pageNo-10%>"> &nbsp < &nbsp </a>	
										<%//1페이지씩 표시.
										for(int i=pageNo;(i<pageNo+10)&&(i<=MaxIndex);i++)
										{%>
											<a href = "freeForum.jsp?pageNo=<%=i%>"><%=i%></a>
										<%}
										//+10이후 더있으면 +10하는 >추가.
										if(pageNo+10<MaxIndex){%><a href="freeForum.jsp?pageNo=<%=pageNo+10%>"> &nbsp > &nbsp </a><%}
									}else{		
									//1붙터 시작하면 10or최대값 까지
										for(int i=1;(i<=MaxIndex);i++){%><a href = "freeForum.jsp?pageNo=<%=i%>"><%=i%></a><%}
									}%>
									<a href="freeForum.jsp?pageNo=<%=MaxIndex%>">>></a>
							</div>
				<%}%>
